...
strBaseDir     = Replace(strCurDir, "tests", "source")

For Each currentPHPversion In CheckedPHPversions
    WshShell.Run "D:\www\App\PHP\PHP56\php.exe D:\www\html_3rdparty\PHP_CodeSniffer\scripts\phpcs -p -v --extensions=php -d date.timezone=""Europe/Bucharest"" --encoding=utf-8 --report=xml --standard=PHPCompatibility --runtime-set testVersion " & currentPHPversion & " " & strBaseDir & " --report-file=" & strCurDir & "\php-code-sniffer\php_" & currentPHPversion & ".xml", 0, True
Next

For Each crtStandard In CheckStandards
    WshShell.Run "D:\www\App\PHP\PHP56\php.exe D:\www\html_3rdparty\PHP_CodeSniffer\scripts\phpcs -p -v --extensions=php -d date.timezone=""Europe/Bucharest"" --encoding=utf-8 --report=xml --standard=" & crtStandard & " " & strBaseDir & " --report-file=" & strCurDir & "\php-code-sniffer\" & crtStandard & ".xml", 0, True
Next
